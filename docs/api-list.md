# Spring AI Alibaba — REST API List

This document catalogs every HTTP endpoint exposed by the Spring `@RestController` / `@Controller` classes in the repository. Endpoints are grouped by module / subsystem.

- **Scanned**: 42 controller files (233 endpoints) across `examples/`, `spring-ai-alibaba-studio`, and `spring-ai-alibaba-admin/*`.
...
|---|---|---|---|
| 1 | [examples/documentation](#1-examplesdocumentation) | 2 | 4 |
| 2 | [examples/multimodal](#2-examplesmultimodal) | 1 | 7 |
| 3 | [spring-ai-alibaba-studio](#3-spring-ai-alibaba-studio) | 7 | 17 |
| 4 | [spring-ai-alibaba-admin-server-openapi](#4-spring-ai-alibaba-admin-server-openapi) | 1 | 5 |
| 5 | [spring-ai-alibaba-admin-server-start (builder)](#5-spring-ai-alibaba-admin-server-start-builder) | 21 | 134 |
| 6 | [spring-ai-alibaba-admin-server-start (generator)](#6-spring-ai-alibaba-admin-server-start-generator) | 4 | 11 |
| 7 | [spring-ai-alibaba-admin-server-start (platform)](#7-spring-ai-alibaba-admin-server-start-platform) | 6 | 55 |
| | **Total** | **42** | **233** |
...
### A2AExampleController
- **Base path**: `/api/a2a`

| Method | Path | Description | Params | Return |
|---|---|---|---|---|
| GET | `/api/a2a/demo` | Run unified A2A demonstration | — | `Map<String, Object>` |

### RemoteMcpToolsExampleController
- **Base path**: `/mcpToolsExample`

| Method | Path | Description | Params | Return |
|---|---|---|---|---|
| POST | `/mcpToolsExample/mcpWithReactSpring` | Call remote MCP tools with Spring Boot example | — | `void` |
| POST | `/mcpToolsExample/mcpWithReact` | Call remote MCP tools without Spring Boot example | — | `void` |
| POST | `/mcpToolsExample/mcpWithSpringChat` | Call remote MCP tools with chat CLI and Spring Boot example | — | `void` |

---

## 2. examples/multimodal
...
| Method | Path | Description | Params | Return |
|---|---|---|---|---|
| POST | `/api/image/from-url` | Describe image from a public URL | `request: ImageUrlRequest (@RequestBody)` | `ResponseEntity<Map<String, String>>` |
| POST | `/api/image/from-resource` | Describe image from uploaded file or classpath resource (multipart) | `image: MultipartFile (@RequestParam), question: String (@RequestParam)` | `ResponseEntity<Map<String, String>>` |
| POST | `/api/image/from-resource` | Describe image from classpath resource (JSON body) | `request: ImageResourceRequest (@RequestBody)` | `ResponseEntity<Map<String, String>>` |
| POST | `/api/vision/agent` | Vision agent with multimodal input | `request: ImageUrlRequest (@RequestBody)` | `ResponseEntity<Map<String, String>>` |
| POST | `/api/creative/agent` | Creative agent for image generation via tools | `request: CreativeRequest (@RequestBody)` | `ResponseEntity<Map<String, Object>>` |
| POST | `/api/audio/tts` | TTS direct synthesis using DashScopeAudioSpeechModel | `request: TtsRequest (@RequestBody)` | `ResponseEntity<?>` |
| GET | `/api/` | Serve the index page | — | `String` |

> The two `POST /api/image/from-resource` rows are differentiated by request content-type (multipart vs. JSON).
...
| Method | Path | Description | Params | Return |
|---|---|---|---|---|
| GET | `/list-apps` | List available applications from dynamic agent registry | — | `List<String>` |

### ChatUiRedirectController
- **Base path**: _(none)_

| Method | Path | Description | Params | Return |
|---|---|---|---|---|
| GET | `/chatui`, `/chatui/` | Redirect to chat UI index page | — | `RedirectView` |
...
| Method | Path | Description | Params | Return |
|---|---|---|---|---|
| POST | `/run_sse` | Execute agent run and stream events via SSE | `request: AgentRunRequest (@RequestBody)` | `Flux<ServerSentEvent<String>>` |
| POST | `/resume_sse` | Resume agent execution with tool feedback | `request: AgentResumeRequest (@RequestBody)` | `Flux<ServerSentEvent<String>>` |
...
| Method | Path | Description | Params | Return |
|---|---|---|---|---|
| GET | `/list-graphs` | List available graphs | — | `List<String>` |
| GET | `/graphs/{graphName}/representation` | Get graph representation in Mermaid format | `graphName: String (@PathVariable)` | `GraphResponse` |
...
| Method | Path | Description | Params | Return |
|---|---|---|---|---|
| POST | `/graph_run_sse` | Execute graph run and stream events via SSE | `request: GraphRunRequest (@RequestBody)` | `Flux<ServerSentEvent<String>>` |
...
| Method | Path | Description | Params | Return |
|---|---|---|---|---|
| GET | `/graphs/{graphName}/users/{userId}/threads/{threadId}` | Retrieve a specific thread for a graph | `graphName: String (@PathVariable), userId: String (@PathVariable), threadId: String (@PathVariable)` | `Thread` |
| GET | `/graphs/{graphName}/users/{userId}/threads` | List all threads for a graph and user | `graphName: String (@PathVariable), userId: String (@PathVariable)` | `List<Thread>` |
| POST | `/graphs/{graphName}/users/{userId}/threads/{threadId}` | Create thread with specific ID for a graph | `graphName: String (@PathVariable), userId: String (@PathVariable), threadId: String (@PathVariable), state: Map<String, Object> (@RequestBody, optional)` | `Thread` |
| POST | `/graphs/{graphName}/users/{userId}/threads` | Create thread with service-generated ID for a graph | `graphName: String (@PathVariable), userId: String (@PathVariable), state: Map<String, Object> (@RequestBody, optional)` | `Thread` |
| DELETE | `/graphs/{graphName}/users/{userId}/threads/{threadId}` | Delete a specific thread for a graph | `graphName: String (@PathVariable), userId: String (@PathVariable), threadId: String (@PathVariable)` | `ResponseEntity<Void>` |
...
| Method | Path | Description | Params | Return |
|---|---|---|---|---|
| GET | `/apps/{appName}/users/{userId}/threads/{threadId}` | Retrieve a specific thread by its ID | `appName: String (@PathVariable), userId: String (@PathVariable), threadId: String (@PathVariable)` | `Thread` |
| GET | `/apps/{appName}/users/{userId}/threads` | List all non-evaluation threads for an app and user | `appName: String (@PathVariable), userId: String (@PathVariable)` | `List<Thread>` |
| POST | `/apps/{appName}/users/{userId}/threads/{threadId}` | Create thread with specific ID provided by client | `appName: String (@PathVariable), userId: String (@PathVariable), threadId: String (@PathVariable), state: Map<String, Object> (@RequestBody, optional)` | `Thread` |
| POST | `/apps/{appName}/users/{userId}/threads` | Create thread with service-generated ID | `appName: String (@PathVariable), userId: String (@PathVariable), state: Map<String, Object> (@RequestBody, optional)` | `Thread` |
| DELETE | `/apps/{appName}/users/{userId}/threads/{threadId}` | Delete a specific thread | `appName: String (@PathVariable), userId: String (@PathVariable), threadId: String (@PathVariable)` | `ResponseEntity<Void>` |

---

## 4. spring-ai-alibaba-admin-server-openapi
...
## 5. spring-ai-alibaba-admin-server-start (builder)
...
## 6. spring-ai-alibaba-admin-server-start (generator)
...
## 7. spring-ai-alibaba-admin-server-start (platform)
